// File: AddKnowledgeNodeType.cs
// Folder: Assets/Scripts/NPC/States/
// Purpose: Transient node that pushes knowledge document GUIDs into AGISKnowledgeState.
//          TypeId: "agis.add_knowledge"
...
{
    public sealed class AddKnowledgeNodeType : IAGISNodeType, IAGISNodeComponentRequirements
    {
        public string TypeId      => "agis.add_knowledge";
        public string DisplayName => "Add Knowledge";
        ...
        private static AGISParamSchema BuildSchema()
        {
            ...
            s.AddOrReplace(new AGISParamSpec { key = "persistent", type = AGISParamType.Bool, defaultValue = AGISValue.FromBool(true), displayName = "Persistent", tooltip = "If false, documents are removed on Exit." });
            for (int i = 0; i < 8; i++)
        ...
        private sealed class Runtime : IAGISNodeRuntime, IAGISNodeSignal
        {
            ...
            public Runtime(AGISExecutionContext ctx, IAGISParamAccessor p)
            {
                ...
                _persistent = p.GetBool("persistent", true);
            ...
            public void Enter()
            {
                ...
                if (_knowledgeState != null)
                    foreach (var id in _docGuids)
                        _knowledgeState.AddDocument(id);
                IsComplete = true;
            }
            ...
            {
                IsComplete = false;
                if (!_persistent && _knowledgeState != null)
                    foreach (var id in _docGuids)
